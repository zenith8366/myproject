...
> 字节、RAM 1282/2048 字节），电脑端四套自动化测试共 43 例、协议解析内核的离线测试
> 52 例全部通过；UNO R3 + 1.77" 屏的显示链路**已实测点亮**。
> **尚未做**：固件在真机上的端到端联调（烧录、串口收发、按钮回传、看门狗恢复），
> 以及本文中标「待实测」的性能数字。
...
| **协议内核离线测试** | `g++ -I tools/proto_test -I firmware/VibePet_UNO tools/test_proto.cpp -o /tmp/proto_test && /tmp/proto_test` | **52 例全过**（转义、`\uXXXX`、嵌套、UTF-8 边界截断、分片重组、超长行丢弃、真实报文）。**不需要硬件** —— 用假 `Arduino.h` 把固件里的解析代码搬到电脑上编译 |
