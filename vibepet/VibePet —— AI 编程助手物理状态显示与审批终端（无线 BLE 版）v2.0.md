...
1. 通过 BLE Nordic UART Service（NUS）无线接收 Claude Code 的状态事件，在 0.96 英寸 TFT 屏幕上显示六种状态提示效果：空闲、工作中、等待审批、完成、出错、失联。
...
graph TB
    ...
    subgraph VibePet 设备
        ...
        E -->|SPI| F[ST7735 TFT<br/>0.96英寸 80×160]
        G[批准按钮] --> E
        H[拒绝按钮] --> E
        I[蜂鸣器] --> E
        J[LED 指示灯] --> E
...
| 项目 | 内容 |
|---|---|
| **推荐型号** | 0.96" ST7735 TFT SPI（80×160） |
| **替代方案** | 1.44" ST7735S SPI（128×128）/ 1.8" ST7735 SPI（128×160） |
| **选型理由** | SPI 接口接线简单，TFT_eSPI 支持完善，价格约 ¥15 |
| **驱动库** | TFT_eSPI（需在 `User_Setup.h` 配置引脚） |
...
| 显示屏 | 0.96" ST7735 TFT | 六种状态 + 审批卡 | ¥15 |
...
| JSON 解析内存溢出 | 低 | 高 | 使用 `StaticJsonDocument`；限制消息长度 |
