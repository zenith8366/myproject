...
- `pc/_smoke_test.py`（5 例）、`pc/_smoke_test_daemon.py`（8 例）、`pc/_smoke_test_state.py`（14 例）—— 冒烟测试，**均不依赖硬件或蓝牙**
...
- `firmware/VibePet/VibePet.ino` —— ESP32-C3 固件（576 行）。**已通过编译验证**（`esp32:esp32` 3.3.11，零警告）
...
**固件尚未在真实硬件上验证**（没有设备）。已确认的只有：能编译、Flash 占用 48%、协议字面量与电脑端逐一对齐、用到的 TFT_eSPI API 签名正确。**未确认**：TFT 初始化序列是否匹配你的模块（`ST7735_INITB` 可能需要换）、接线与引脚、真实 BLE 行为、屏幕布局的实际观感。
