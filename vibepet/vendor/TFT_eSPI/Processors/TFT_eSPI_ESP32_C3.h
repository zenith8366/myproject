...
// Fix IDF problems with ESP32C3
  ...
  //
  // ── VibePet 修正（2026-09-23）─────────────────────────────────────────
  // 原来这里是 #ifndef。在 ESP32 核心 2.x（IDF 4.x）上没问题，因为那时
  // IDF 根本没定义 REG_SPI_BASE。
  //
  // 但核心 3.x（IDF 5.x）的 soc/soc.h 自己定义了一个兼容垫片：
  //     REG_SPI_BASE(i)  →  ((i)==2) ? DR_REG_SPI2_BASE : 0
  // 它只认 IDF 4.x 时代的旧枚举值。而 IDF 5.x 已把 SPI2_HOST 从 2 改成 1，
  // 垫片匹配失败返回 0 —— 于是 SPI_USER_REG 等寄存器指针落在 0x10，
  // _spi_user 一写就触发 Store access fault，固件启动即崩溃重启，
  // 表现为「白屏 + 蜂鸣器乱响 + 无限重启 + 蓝牙搜不到」。
  //
  // #ifndef 正是帮凶：IDF 5.x 既然「定义过」了，下面这句正确的写法就被
  // 跳过。改用 #undef 夺回定义权。#undef 一个不存在的宏是合法的空操作，
  // 因此在旧核心上行为不变。
  // ─────────────────────────────────────────────────────────────────────
  #undef REG_SPI_BASE
...
////////////////////////////////////////////////////////////////////////////////////////
      ...
      #if (TFT_MISO == -1)
        // ── VibePet 修正（2026-09-23）───────────────────────────────────
        // 原来这里无条件把 TFT_MISO 改成 TFT_MOSI，于是 TFT_eSPI::init()
        // 里的
        //     spi.begin(TFT_SCLK, TFT_MISO, TFT_MOSI, -1)
        // 展开成 spi.begin(4, 6, 6, -1) —— MISO 和 MOSI 撞在同一个 GPIO 上。
        //
        // 核心 2.x 上这无所谓。核心 3.x 引入了 periman 引脚外设管理器：
        // spiAttachMISO(_spi, 6) 先把 GPIO6 认领给 MISO，紧接着
        // spiAttachMOSI(_spi, 6) 要认领同一个脚就冲突失败，MOSI 没挂上，
        // SPI 实际发不出数据。
        //
        // 而库靠轮询 while (*_spi_cmd & SPI_USR) 判断发送完成 —— 数据发不
        // 出去这一位就永不清零，于是死循环卡在 tft.init() 里。因为核心 3.x
        // 的任务看门狗默认不覆盖 loopTask，板子既不复位也无输出，看起来
        // 像「死机」（屏幕白屏、蓝牙永远起不来）。
        //
        // 核心 3.x 下保持 TFT_MISO = -1（即不使用 MISO 脚）即可。
        // ─────────────────────────────────────────────────────────────────
